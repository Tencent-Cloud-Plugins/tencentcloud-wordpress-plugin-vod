    ...
    class TencentWordpressPluginsSettingActions
    {
        ...
        private static function initHooks()
        {
            ...
            add_action('admin_menu',
                array('TencentWordpressPluginsSettingActions', 'addTencentWordpressCommonSettingPage')
            );

            // 保存插件公共配置信息
            add_action('wp_ajax_save_tencent_wordpress_common_options',
                array('TencentWordpressPluginsSettingActions', 'saveTencentWordpressCommonOptions')
            );

            // 将数据库wp_tencent_wordpress_options表中插件的状态为false
            add_action('wp_ajax_close_tencent_wordpress_plugin',
                array('TencentWordpressPluginsSettingActions', 'closeTencentWordpressPlugin')
            );

            // 将数据库wp_tencent_wordpress_options表中插件的状态为true
            add_action('wp_ajax_open_tencent_wordpress_plugin',
                array('TencentWordpressPluginsSettingActions', 'openTencentWordpressPlugin')
            );
        }
        ...
        public static function addTencentWordpressCommonSettingPage()
        {
            ...
            if ($exists === false) {
                $pagehook = add_menu_page(
                    __('Tencent Cloud Settings', 'tencentcloud-vod-v2'),
	                __('Tencent Cloud Settings', 'tencentcloud-vod-v2'),
                    'manage_options',
                ...
                add_action('admin_print_styles-' . $pagehook,
                    array('TencentWordpressPluginsSettingActions', 'tencent_wordpress_common_loadCssForPage'));
            }
        }
        ...
        public static function sendUserSaveCommonSecretInfo()
        {
            ...
            if (isset($common_option['secret_id']) && isset($common_option['secret_key'])) {
                $static_data['data']['uin'] = self::getUserUinBySecret(
                    $common_option['secret_id'],
                    $common_option['secret_key']
                );
            }
