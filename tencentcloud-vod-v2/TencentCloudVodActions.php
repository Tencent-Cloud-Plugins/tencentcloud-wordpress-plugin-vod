...
if (!is_file(TENCENT_WORDPRESS_VOD_DIR . 'vendor/autoload.php')) {
    wp_die(
	    __('Missing dependency file, please run `composer install` first', 'tencentcloud-vod-v2'),
...
class TencentCloudVodActions
{
    ...
    {
        if ($file == plugin_basename(TENCENT_WORDPRESS_VOD_DIR . 'tencentcloud-vod-v2.php')) {
    ...
    public static function checkMustParams($secretID, $secretKey, $SubAppId, $transcode, $secretCustom)
    {
        if ($secretCustom == '1') {
            if (empty($secretID)) {
                return __('SecretId not set.', 'tencentcloud-vod-v2');
            }
            if (empty($secretKey)) {
	            return __('SecretKey not set.', 'tencentcloud-vod-v2');
            }
        }
        if (empty($SubAppId)) {
	        return __('SubAppId not set.', 'tencentcloud-vod-v2');
        }
        return true;
    }
    ...
    public function checkVodSettings() {
        ...
        try {
            ...
            wp_send_json_success(array(
				'msg' => __('Configuration verification succeeded', 'tencentcloud-vod-v2'),
				'info' => $resp->toJsonString()));
        } catch (TencentCloudSDKException $e) {
            wp_send_json_error(array(
				'msg' => __('Configuration verification failed', 'tencentcloud-vod-v2'),
				'info' => $e));
        }
    }
    ...
    public function updateVodSettings()
    {
        if (!current_user_can('manage_options')) {
            wp_send_json_error(array('msg' => __('The current user has no permission.', 'tencentcloud-vod-v2')));
        }
        $vodOptionsSettings = new TencentCloudVodOptions();
        $vodOptionsSettings->setSecretID(sanitize_text_field($_POST['secretId']));
        $vodOptionsSettings->setSecretKey(sanitize_text_field($_POST['secretKey']));
        ...
        wp_send_json_success(array('msg' => __('Save ok', 'tencentcloud-vod-v2')));
    ...
    public static function activatePlugin()
    {
        ...
        $plugin = array(
            ...
            'nick_name' => __('Tencent Cloud VOD plugin', 'tencentcloud-vod-v2'),
            'plugin_dir' => 'tencentcloud-vod-v2/tencentcloud-vod-v2.php',
